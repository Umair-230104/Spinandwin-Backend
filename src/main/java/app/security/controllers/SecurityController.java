...
public class SecurityController implements ISecurityController {
    ...
    public static SecurityController getInstance() { // Singleton because we don't want multiple instances of the same class
        ...
        securityDAO = new SecurityDAO(HibernateConfig.getEntityManagerFactory("invitationsecuritydb")); // ændre db navn hvis det er forkert
        return instance;
    }
    ...
    public Handler updateUser() {
        return ctx -> {
            ...
            try {
                User updated = securityDAO.updateUser(username, newPassword);
                ctx.status(200).json(objectMapper.createObjectNode()
                        .put("msg", "Password updated for user: " + updated.getUsername()));
            } catch (EntityNotFoundException e) {
                ctx.status(404).json(objectMapper.createObjectNode()
    ...
    public Handler getAllUsers() {
        return ctx -> {
            ctx.json(securityDAO.getAllUsers());
        };
    }
    ...
    public Handler deleteUser() {
        return ctx -> {
            String username = ctx.pathParam("username"); // fx /api/users/:username

            try {
                securityDAO.deleteUser(username);
                ctx.status(200).json(objectMapper.createObjectNode()
                        .put("msg", "User deleted: " + username));
            } catch (EntityNotFoundException e) {
                ctx.status(404).json(objectMapper.createObjectNode()
                        .put("msg", "User not found"));
            } catch (ApiException e) {
